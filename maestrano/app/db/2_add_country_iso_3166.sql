...
/* 
 * VENDOR COUNTRY
 * Note: the vtiger_field 'fieldname' has to match the picklist 'name'
 */
UPDATE  `vtiger_field` SET  `uitype` =  '15', `fieldname` = 'vendorcountry' WHERE  `vtiger_field`.`fieldid` = 304;
UPDATE  `vtiger_picklist_seq` SET  `id` =  `id` + 1;

INSERT INTO  `vtiger_picklist` (`picklistid` , `name`)
SELECT id,  "vendorcountry" AS name
FROM `vtiger_picklist_seq`;

CREATE TABLE IF NOT EXISTS `vtiger_vendorcountry` (
  `vendorcountry` varchar(200) NOT NULL,
  `presence` int(1) NOT NULL DEFAULT '1',
  `picklist_valueid` int(19) NOT NULL,
  `vendorcountryid` int(11) NOT NULL,
  `sortorderid` int(10) DEFAULT NULL,
  PRIMARY KEY (`vendorcountryid`),
  UNIQUE KEY `vendorcountry_vendorcountry_idx` (`vendorcountry`)
);

CREATE TABLE IF NOT EXISTS `vtiger_vendorcountry_seq` (
  `id` int(11) NOT NULL
);

INSERT INTO  `vtiger_vendorcountry_seq` (`id`) VALUES ('242');

INSERT INTO `vtiger_vendorcountry` (`vendorcountry`, `presence`, `picklist_valueid`, `vendorcountryid`, `sortorderid`) VALUES
('--None--', 1, 288, 1, 0),
('Afghanistan', 1, 289, 2, 1),
('Aland Islands', 1, 290, 3, 2),
('Albania', 1, 291, 4, 3),
...
UPDATE `vtiger_vendorcountry` SET `picklist_valueid`=`picklist_valueid`-287+(SELECT id
    FROM vtiger_picklistvalues_seq);
...
INSERT INTO `vtiger_role2picklist`(`roleid`, `picklistvalueid`, `picklistid`, `sortid`)
SELECT vr.roleid, vbc.picklist_valueid, picklist_seq.id AS picklistid, vbc.sortorderid
FROM vtiger_role vr, vtiger_vendorcountry vbc, (
    SELECT id
    FROM vtiger_picklist_seq
    ) AS picklist_seq;